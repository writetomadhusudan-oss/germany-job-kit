/* Netherlands CV Builder — Dutch-format CV, live preview, optional photo, sample data. Client-side only. */
...
function render() {
  ...
  if (summary) h += '<h2>Profiel</h2><p>' + esc(summary) + '</p>';
  if (exps.length) {
    h += '<h2>Werkervaring</h2>';
    exps.forEach(e => {
  ...
  if (skills.length) h += '<h2>Vaardigheden</h2><p>' + esc(skills.join(', ')) + '</p>';
  if (certs.length) h += '<h2>Certificaten</h2><ul>' + certs.map(c => '<li>' + esc(c) + '</li>').join('') + '</ul>';
  if (langs.length) h += '<h2>Talen</h2><ul>' + langs.map(l => '<li>' + esc(l) + '</li>').join('') + '</ul>';
  h += '<div style="clear:both"></div>';
  $('cvPreview').innerHTML = h;
}

['f_name','f_title','f_city','f_phone','f_email','f_linkedin','f_summary','f_skills','f_certs','f_langs']
