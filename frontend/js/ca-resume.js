/* Canada Resume Builder — Canadian-format resume, live preview, sample data. Client-side only. */
...
function addExp(data) {
  ...
  div.innerHTML =
    ...
    '<label>Accomplishments <span class="hint">(one per line — start with action verbs, quantify results)</span>' +
...
function addEdu(data) {
  ...
  div.style.cssText = 'border:1px solid var(--line);border-radius:8px;padding:12px;margin-bottom:10px';
  div.innerHTML =
    '<label>Credential <input type="text" data-k="degree" value="' + esc(data.degree || '') + '"></label>' +
...
function render() {
  ...
  const highlights = v('f_highlights').split('\n').map(s => s.trim()).filter(Boolean);
  ...
  if (highlights.length) h += '<h2>Highlights of Qualifications</h2><ul>' + highlights.map(x => '<li>' + esc(x) + '</li>').join('') + '</ul>';
  if (exps.length) {
  ...
  if (skills.length) h += '<h2>Technical Skills</h2><p>' + esc(skills.join(', ')) + '</p>';
  if (certs.length) h += '<h2>Certifications</h2><ul>' + certs.map(c => '<li>' + esc(c) + '</li>').join('') + '</ul>';
  $('cvPreview').innerHTML = h;
}

['f_name','f_title','f_city','f_phone','f_email','f_linkedin','f_summary','f_highlights','f_skills','f_certs']
...
$('fillSample').addEventListener('click', () => {
  ...
  $('f_city').value = 'Toronto, ON';
  $('f_phone').value = '+1 416 555 0132';
  $('f_email').value = 'priya.sharma@example.com';
  $('f_linkedin').value = 'linkedin.com/in/priyasharma';
  $('f_summary').value = 'SAP SuccessFactors consultant with 8 years of experience across Employee Central and Recruiting. Led 5 full-cycle implementations for manufacturing and retail clients (50–5,000 employees). Authorized to work in Canada; available immediately.';
  $('f_highlights').value = '8 years SAP SuccessFactors Employee Central & Recruiting\nLed 5 full-cycle implementations (50–5,000 employees)\nFluent English; strong client-facing workshop facilitation';
