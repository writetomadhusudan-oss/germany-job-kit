/* UK CV Builder — UK-format CV, live preview, sample data. Client-side only. */
...
function addExp(data) {
  ...
  div.innerHTML =
    ...
    '<div class="row"><label>Employer <input type="text" data-k="company" value="' + esc(data.company || '') + '"></label>' +
    ...
    '<label>Responsibilities & achievements <span class="hint">(one per line — start with action verbs, quantify results)</span>' +
...
function render() {
  ...
  const skills = v('f_skills').split('\n').map(s => s.trim()).filter(Boolean);
  const certs = v('f_certs').split('\n').map(s => s.trim()).filter(Boolean);
  const statement = v('f_statement');

  let h = '<h1>' + esc(name) + '</h1>';
  if (v('f_title')) h += '<p><strong>' + esc(v('f_title')) + '</strong></p>';
  if (contact) h += '<div class="contact">' + contact + '</div>';
  if (statement) h += '<h2>Personal Statement</h2><p>' + esc(statement) + '</p>';
  if (skills.length) h += '<h2>Key Skills</h2><ul>' + skills.map(x => '<li>' + esc(x) + '</li>').join('') + '</ul>';
  if (exps.length) {
...
['f_name','f_title','f_city','f_phone','f_email','f_linkedin','f_statement','f_skills','f_certs']
...
$('fillSample').addEventListener('click', () => {
  ...
  $('f_city').value = 'London';
  $('f_phone').value = '+44 7700 900123';
  $('f_email').value = 'priya.sharma@example.com';
  $('f_linkedin').value = 'linkedin.com/in/priyasharma';
  $('f_statement').value = 'SAP SuccessFactors consultant with 8 years of experience across Employee Central and Recruiting. Proven record of leading full-cycle implementations for manufacturing and retail clients, delivering on time and under budget. Eligible for UK Skilled Worker sponsorship; available to relocate.';
  $('f_skills').value = 'SAP SuccessFactors Employee Central & Recruiting\nFull-cycle HRIS implementations\nClient-facing workshops and requirements gathering\nData migration and reporting';
  ...
  addExp({title:'Senior SAP SuccessFactors Consultant', company:'TechMahindra Consulting', city:'Bengaluru, India',
    ...
    bullets:'Led Employee Central implementation for a 5,000-employee retail client; go-live delivered two weeks early\nReduced onboarding ticket volume by 35% by redesigning recruiting workflows\nMentored 4 junior consultants and ran client workshops in English'});
  addExp({title:'SAP HCM Consultant', company:'Infosys BPM', city:'Bengaluru, India',
    start:'07/2017', end:'05/2021',
    bullets:'Supported payroll and time management for 12,000 employees across three countries\nAutomated monthly HR reports with SQL, saving approximately 20 hours per month'});
